...
//        main.makeHpLevelPropertiesCode(BorgUtils.getLevelPropertiesStartAddress(0xa09), 310, 5);
//        main.makeAmmoLevelPropertiesCode(BorgUtils.getLevelPropertiesStartAddress(0xa09), 240);
        LevelProperties[] levelProperties = BorgUtils.getLevelProperties(0x609);
        for (int x = 0; x < levelProperties.length; x++) {
            levelProperties[x].setHp(390 + (x * 5));
            levelProperties[x].setbAmmoStart(8);
            levelProperties[x].setbAmmoMax(8);
            levelProperties[x].setbAmmoType(1);
            levelProperties[x].setbAmmoReload(180);

            levelProperties[x].setxAmmoStart(3);
            levelProperties[x].setxAmmoMax(3);
            levelProperties[x].setxAmmoType(1);
            levelProperties[x].setxAmmoReload(360);
        }
//        main.makeAmmoLevelPropertiesCode(BorgUtils.getLevelPropertiesStartAddress(0x61d), levelProperties);
...
//        }
    ...
    private void makeAmmoLevelPropertiesCode(int startAddress, LevelProperties[] levelProperties) {
        int levelCount = 20;
        int levelSize = 18;
        int dataSize = 2;
        for (int property = 0; property < levelSize/dataSize; property++) {
            System.out.println(LevelProperties.getPropertyNameByIndex(property));
            int tempStartAddress = startAddress + (property * dataSize);
            for (int level = 0; level < levelCount; level++) {
                int codeLine = (0x00FFFFFF & tempStartAddress) | 0x2000000;
                System.out.println(String.format("%08x %08x", codeLine, levelProperties[level].getPropertyValueByIndex(property)));
